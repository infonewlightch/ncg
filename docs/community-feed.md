...
No user content was sent to a provider during this check. At the time of this earlier check, Netlify `/api/translate` was an explicit 503 stub. The subsequent adapter connection and verification limits are documented in [community translation](community-translation.md). Bible translation assistance uses a separate endpoint and is not evidence of connected community or chat translation.
