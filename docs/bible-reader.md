...
Publisher HTML indexes confirm poronbv and porbr2018 contain the 39 OT + 27 NT books; porbrbsl contains 81 books including those 66; porblt and portft contain only 27 NT books despite the broad title on one source. The verified book counts are in `src/data/bible-coverage.json`. Full 66-book editions sort before NT-only editions, with the source-certified Biblica ONBV first. Explicit stored version preferences are respected. This metadata check does not certify translation quality or verify every verse. Copyright notices were checked for these five redistributable entries; por-WBTC remains source-only.
