...
- [x] 페이지 파일 로딩 실패와 렌더 오류를 구분하는 복구 화면. 오프라인 안내·명시적 새로고침·홈 이동, 언어 창 로딩/실패 시 닫기와 기존 영상 입력 유지. 실제 로컬 404→사용자 새로고침 복구와 PC/모바일 확인, 285 tests/50 files 및 build 통과. 이전 버전으로 이미 열려 있는 화면은 한 번 새로고침이 필요할 수 있음.
