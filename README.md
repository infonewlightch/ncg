...
- 한국어/영어/스페인어 화면과 사전 생성한 포르투갈어·중국어 간체/번체·태국어·아랍어·페르시아어·힌디어·프랑스어 화면. 나머지 언어는 제공 가능한 범위에서 화면 문구를 자동 번역하며, 실패·미지원 상태를 표시합니다. 생성 번역은 원어민 검수 전입니다.
...
- `#/school/quiz`: ko3,011/en3,004/th3,008문제. 난이도·구약/신약/책·문항 수·선택적20초 타이머·힌트·연속정답·오답복습. 점수는 기기에 저장하며 로그인하면 계정 동기화를 지원합니다.
...
- 친구·채팅·이메일 로그인·프로필과 관리자 화면이 있습니다. 교회 운영자 이메일의 실제 가입·메일 수신·인증 복귀·마이페이지 로그인을 확인했습니다. Google 제공자 연결, 일반 회원용 SMTP, 실제 두 회원 통합 검증은 남아 있습니다.
- 영어 WEBP 전체 66권을 약 9.7 MB와 앱 화면 용량으로 기기에 저장하는 기능을 구현했습니다. 파일 해시를 확인하며 중단 후 이어받습니다. 서버 중단 후 브라우저 읽기를 확인했고, 실기기 비행기 모드 검증은 남아 있습니다.
- 문의하기는 실제 사이트 제출 → Netlify 비공개 접수함 → 교회 Gmail 알림 수신을 확인했습니다.

화면 문구 768개와 번역 누락·변수 보존을 빌드 때 검사합니다. [언어별 제공 범위와 번역 절차](docs/i18n.md)를 참고하세요.
...
- NCG 전용 Supabase `yndtcpsajhmnyeqeozju`에 공개 클라이언트를 연결하고 001–011 마이그레이션을 적용했습니다. 기존 운영 프로젝트에 재실행하지 않습니다. 최신 실운영 연결 결과는 [운영 기록](docs/operations/connection-status.md)을 확인하세요.
- 새 프로젝트에만 `supabase/migrations`의 001–011을 순서대로 적용합니다. 회원·QT·알림·공동체·진도·영상·QT 장절 범위·성경 책갈피·퀴즈·영상 책갈피·번역 이용량 제한을 포함합니다. 기존 다른 앱 DB에 적용하지 않습니다.
...
학습/QT 완료와 취소, 성경 책갈피, 퀴즈 점수, 교회 영상 책갈피는 계정별 변경분을 동기화하고 같은 요청의 재전송은 중복 적용하지 않습니다. 연결이 끊기면 기기에 보관 후 다시 시도합니다. 개인이 추가한 영상은 기기에 저장합니다.
...
where lower(email) = 'infonewlightch@gmail.com'
...
- eBible/getBible의 언어·역본 카탈로그와 허용된 본문 공급원을 연결했습니다. 카탈로그 개수는 모든 역본의 본문 검증·사용허가 완료 개수가 아닙니다.
- Netlify 함수에서 UI 번역, 영어 WEBP 기반 참고 번역, 공동체/친구 대화 번역을 기존 AI Gateway와 연결했습니다. 로그인·RLS·원문 접근 권한·공유 이용량 제한을 적용합니다. 참고 번역은 검증된 성경 역본과 구별하며, 실제 두 회원의 비공개 채팅 번역은 아직 미검증입니다. [운영 서버 문서](docs/production-server.md)를 참고하세요.

## 매일 QT 알림

기기별 현지 시각/시간대 설정, 서버 구독 소유권, 매일 중복 방지, 만료 구독 비활성화, SW 알림 수신·날짜별 QT 열기를 구현했습니다. Supabase 스키마와 Netlify 예약 함수는 배포됐지만 **VAPID·서버 비밀값·발송 활성화가 남아 있으며 알림은 아직 발송하지 않았습니다.** 설정·제약·검증 순서는 [알림 운영 문서](docs/push-reminders.md)를 따릅니다.
...
공개 미리보기는 GitHub main과 연결된 Netlify 배포이며 서버 함수 6개를 포함합니다. 공식 도메인·HTTPS·OG 이미지·GitHub 백업을 확인했습니다. 남은 핵심은 Google 로그인·일반 회원 SMTP, 관리자 연결, 개역개정 사용허가, 언어별 콘텐츠 검수, QT 교회 검수·발행, 푸시 활성화·실기기 수신과 실제 두 회원 통합 검증입니다. 최신 결과는 [운영 연결 기록](docs/operations/connection-status.md), 지속 기록은 로컬 `docs/WORKLOG.md`를 봅니다.

## 운영 주체

대한예수교장로회(합동) 경기동중노회 · 새빛교회

대한민국 경기도 광명시 하안동 200-1, 4층 · infonewlightch@gmail.com
