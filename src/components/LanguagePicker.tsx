...
export default function LanguagePicker({onClose,onSelect}:{onClose:()=>void;onSelect?:(code:string)=>void}){
 const {state,update,t}=useApp();const [languages,setLanguages]=useState<Language[]>([]);const [limit,setLimit]=useState(60);const [query,setQuery]=useState('');const [custom,setCustom]=useState('');const [error,setError]=useState('');const [selecting,setSelecting]=useState('');const [failedLanguage,setFailedLanguage]=useState('');const selectionPending=useRef(false);
 ...
 async function select(code:string){
  ...
  if(onSelect){onSelect(code);onClose();return;}
  selectionPending.current=true;setSelecting(code);setFailedLanguage('');
  try{
   if(interfaceSeed(code)&&!await prepareInterface(interfaceLanguage(code)))throw Error('language_unavailable');
   if(!mounted.current)return;update(s=>({...s,language:code}));onClose();
  }catch{if(mounted.current)setFailedLanguage(code);}
  finally{selectionPending.current=false;if(mounted.current)setSelecting('');}
 }
