...
export function SharedFeed({topic,category,mine=false,onCompose}:{topic?:string;category?:string;mine?:boolean;onCompose?:()=>void}){
 ...
 const local=state.posts.filter(p=>(mine||topic?p.topic===topic||mine:!p.topic)&&(!category||category==='all'||p.category===category));
 useEffect(()=>{
  ...
  if(canLoad)Promise.resolve(supabase!.rpc('ncg_feed',{scope,qt_id:qtId,kind:category&&category!=='all'?category:null}).abortSignal(c.signal)).then(({data,error})=>{
 ...
 },[canLoad,scope,qtId,category,reload,session?.user.id]);
 async function loadMore(){
  ...
  try{
   const {data,error}=await supabase.rpc('ncg_feed',{scope,qt_id:qtId,kind:category&&category!=='all'?category:null,before_time:last.createdAt,before_id:last.id}).abortSignal(c.signal);
