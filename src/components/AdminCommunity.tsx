import {CountryLabel} from './CountrySelect';
import {useEffect,useState} from 'react';
import {supabase} from '../auth';
type ReviewPost={id:string;body:string;author:string;nationality:string;language:string;created_at:string;topic_id:string|null;parent_post_id:string|null};
export function AdminCommunity(){
 const [posts,setPosts]=useState<ReviewPost[]>([]);const [error,setError]=useState('');const [busy,setBusy]=useState(false);
 async function load(){const {data,error}=await supabase!.from('ncg_posts').select('id,body,author,nationality,language,created_at,topic_id,parent_post_id').eq('status','pending').order('created_at').limit(100);if(error)setError('공동체 검토 목록을 불러오지 못했습니다. 004 마이그레이션과 서버 연결을 확인해주세요.');else{setPosts(data||[]);setError('');}}
 useEffect(()=>{void load();},[]);
