...
import {chapterOf,firstVerse,verseInSelection,type BibleIndex} from '../core/bible';
...
export function BibleNavigator({index,passage,onSelect,onRead}:{index:BibleIndex;passage:string;onSelect:(id:string)=>void;onRead?:()=>void}){
 ...
 return <div className="bible-navigator" ref={root}>
  ...
  <div className="bible-canon-tabs" aria-label={t('성경 구분','Testament')}>{[['all',t('전체','All')],['old_testament',t('구약','Old Testament')],['new_testament',t('신약','New Testament')]].map(([id,label])=><button type="button" key={id} data-canon={id} aria-pressed={canon===id} onClick={()=>setCanon(id)}>{label}</button>)}</div>
  <div className="bible-scroll-columns">
   <div className="bible-scroll-column bible-books-column"><h3>{t('성경','Book')}</h3><div className="bible-scroll-list" aria-label={t('성경 목록','Bible books')}>{books.map(b=>{const alias=names.find(n=>n.code===(b.id==='NAM'?'NAH':b.id));const title=state.ui==='ko'&&alias?alias.ko:b.title;return <button type="button" key={b.id} data-canon={b.canon} aria-current={book?.id===b.id?'true':undefined} onClick={()=>b.chapters[0]&&onSelect(firstVerse(b.chapters[0].passage_id))}><b dir="auto">{title}</b>{title!==b.title&&<small dir="auto">{b.title}</small>}</button>;})}{!books.length&&<p className="navigator-empty">{t('일치하는 성경이 없습니다.','No matching books.')}</p>}</div></div>
   <div className="bible-scroll-column" data-canon={book?.canon}><h3>{t('장','Chapter')}</h3><div className="bible-scroll-list" aria-label={t('장 목록','Chapters')}>{book?.chapters.map(c=><button type="button" key={c.passage_id} aria-label={`${book.title} ${c.title}`} aria-current={chapter?.passage_id===c.passage_id?'true':undefined} onClick={()=>onSelect(firstVerse(c.passage_id))}>{c.title}</button>)}</div></div>
   <div className="bible-scroll-column" data-canon={book?.canon}><h3>{t('절','Verse')}</h3><div className="bible-scroll-list" aria-label={t('절 목록','Verses')}>{chapter?.verses.map(v=><button type="button" key={v.passage_id} aria-label={`${book?.title} ${chapter.title}:${v.title}`} aria-current={verseInSelection(firstVerse(passage),String(v.id))?'true':undefined} onClick={()=>{onSelect(v.passage_id);onRead?.();}}>{v.title}</button>)}</div></div>
  </div>
