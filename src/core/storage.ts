...
export const initialState:AppState={version:1,ui:'ko',language:'ko',lowData:false,profile:{name:'',nationality:''},videos:[],posts:[],bookmarks:[],completed:[],prayed:[],requests:[],quiz:{answered:0,correct:0,rounds:0,bestScore:0,bestStreak:0},progressSync:{baseline:[],pending:null,revision:0}};
...
export function readState(raw:string|null):AppState {
  try {
    const s=JSON.parse(raw||'null');
    if(!s||s.version!==1) return structuredClone(initialState);
    ...
    return {...initialState,ui:interfaceLanguage(language),language,lowData:s.lowData===true,
      ...
      quiz:{answered:count(s.quiz?.answered),correct:Math.min(count(s.quiz?.correct),count(s.quiz?.answered)),rounds:count(s.quiz?.rounds),bestScore:Math.min(600,count(s.quiz?.bestScore)),bestStreak:Math.min(30,count(s.quiz?.bestStreak))},
