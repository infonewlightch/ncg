...
import {canonicalEbibleBook,ebibleBookCode} from './bible-source-codes';
...
const catalog=async()=>({default:{versions:(await import('../data/bible-reader-catalogue.json')).default.ebible}});
...
const editionLanguage=(entry:Entry)=>entry.id==='cmn-cu89t'?'zh-Hant':entry.id==='cmn-cu89s'?'zh-Hans':canonical(entry.language);
...
export function parseEbibleBooks(doc:Document):BibleBook[]{
 const used=new Set<string>();return Array.from(doc.querySelectorAll('.bookList a[href]')).flatMap(a=>{const match=/^([A-Z0-9]{3})(\d{2,3})\.htm$/.exec(a.getAttribute('href')||'');if(!match)return [];const id=canonicalEbibleBook(match[1]);if(!canonicalBook(id)||used.has(id))return [];used.add(id);const title=normalize(a.textContent||id),chapter=Math.max(1,Number(match[2]));return [{id,title,full_title:title,abbreviation:id,canon:a.classList.contains('oo')?'old_testament':a.classList.contains('nn')?'new_testament':'other',chaptersKnown:false,chapters:[{id:chapter,title:chapter,passage_id:`${id}.${chapter}`,verses:[],versesKnown:false,sourceFile:match[0]}]}];});
}
export function parseEbibleChapters(doc:Document,book:string):BibleChapter[]{
 const used=new Set<string>();return Array.from(doc.querySelectorAll('.tnav a[href]')).flatMap(a=>{const match=/^([A-Z0-9]{3})(\d{2,3})\.htm$/.exec(a.getAttribute('href')||'');if(!match||canonicalEbibleBook(match[1])!==book||!canonicalChapter(book,Number(match[2]))||used.has(match[0]))return [];used.add(match[0]);const id=Number(match[2]);return [{id,title:normalize(a.textContent||'')||id,passage_id:`${book}.${id}`,verses:[],versesKnown:false,sourceFile:match[0]}];});
}
...
export async function ebibleRequest<T>(resource:string,params:Record<string,string>,signal:AbortSignal):Promise<T>{
 ...
 if(resource==='index'){
  const books=parseEbibleBooks(await html(v.id,'index.htm',signal));if(!books.length)throw Error('bible_source_unavailable');
  const selected=books.find(b=>b.id===params.passage?.split('.')[0])||books[0];const chapters=parseEbibleChapters(await html(v.id,`${ebibleBookCode(selected.id)}.htm`,signal),selected.id);if(!chapters.length)throw Error('bible_source_unavailable');selected.chapters=chapters;selected.chaptersKnown=true;
  return {text_direction:v.direction==='rtl'?'rtl':'ltr',books,license:copyright} as BibleIndex as T;
 }
 if(resource==='book'){
  if(!/^[A-Z0-9]{3}$/.test(params.book))throw Error('invalid_reference');const chapters=parseEbibleChapters(await html(v.id,`${ebibleBookCode(params.book)}.htm`,signal),params.book);if(!chapters.length)throw Error('bible_source_unavailable');return chapters as T;
 }
 if(resource==='passage'){
  const match=/^([A-Z0-9]{3})\.([1-9]\d{0,2})$/.exec(params.passage);if(!match)throw Error('invalid_reference');
  const chapters=parseEbibleChapters(await html(v.id,`${ebibleBookCode(match[1])}.htm`,signal),match[1]);const chapter=chapters.find(c=>c.passage_id===params.passage);if(!chapter?.sourceFile)throw Error('passage_unavailable');
