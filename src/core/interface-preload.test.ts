...
const messages=()=>Object.fromEntries(interfaceSource.map(row=>[row.en,row.en==='Home'?'Startseite':row.en]));
...
it('retries a failed static preload on selection without requesting a model',async()=>{
 seeds.load.mockRejectedValueOnce(Error('temporary network failure')).mockResolvedValue({default:{language:'de',messages:messages(),sourceContext:context}});
 expect(await runtime.prepareInterface('de')).toBe(false);expect(await runtime.prepareInterface('de')).toBe(true);
...
it('does not reintroduce old-context text from a static pack into the persistent cache',async()=>{
 localStorage.setItem('ncg:interface:v2:de',JSON.stringify([{en:'Bible',ko:context.Bible,text:'Bibel'}]));
 seeds.load.mockResolvedValue({default:{language:'de',messages:{...messages(),Home:'Stale',Bible:'Invalid replacement'},sourceContext:{...context,Home:'changed'}}});
 expect(await runtime.prepareInterface('de')).toBe(false);
 ...
 expect(runtime.runtimeInterfaceStatus('de')).toBe('unavailable');
});
