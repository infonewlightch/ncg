...
export async function getBibleRequest<T>(resource:string,params:Record<string,string>,signal:AbortSignal):Promise<T>{
 const {default:data}=await catalogue();const edition=data.versions.find(v=>`eb-gb-${v.id}`===params.version);if(!edition)throw Error('bible_source_unavailable');
 if(resource==='index'){
  const source=await request(edition.id,'index',signal) as Record<string,{nr:number;name:string}>;
  const books:BibleBook[]=Object.values(source).filter(b=>Number.isInteger(b.nr)&&b.nr>=1&&b.nr<=99&&typeof b.name==='string').sort((a,b)=>a.nr-b.nr).map(b=>({id:codeOf(b.nr),title:b.name,full_title:b.name,abbreviation:codeOf(b.nr),canon:b.nr<=39?'old_testament':b.nr<=66?'new_testament':'other',chaptersKnown:false,chapters:[{id:1,title:1,passage_id:`${codeOf(b.nr)}.1`,verses:[],versesKnown:false}]}));
