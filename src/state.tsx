...
type Store={quizStatus:QuizSyncStatus;setQuizStatus:(status:QuizSyncStatus)=>void;bookmarkStatus:BookmarkStatus;setBookmarkStatus:(status:BookmarkStatus)=>void;state:AppState;update:(fn:(s:AppState)=>AppState)=>void;t:(ko:string,en:string)=>string;storageError:boolean;notice:string;notify:(message:string)=>void};
...
export function Provider({children,storageKey='ncg:v1'}:{children:ReactNode;storageKey?:string}){
  ...
  return <Context.Provider value={{state,update,t,storageError,notice,notify:setNotice,bookmarkStatus,setBookmarkStatus,quizStatus,setQuizStatus}}>{children}</Context.Provider>;
}
