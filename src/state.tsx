...
import {interfaceSnapshot,subscribeInterface,retryInterface,runtimeInterfaceStatus,type InterfaceStatus} from './core/interface-runtime';
import {hasInterfaceTranslation,interfaceLanguage} from './core/languages';
import {translateInterface} from './core/interface';

type Store={interfaceStatus:InterfaceStatus;retryInterface:()=>void;videoBookmarkStatus:BookmarkStatus;setVideoBookmarkStatus:(status:BookmarkStatus)=>void;quizStatus:QuizSyncStatus;setQuizStatus:(status:QuizSyncStatus)=>void;bookmarkStatus:BookmarkStatus;setBookmarkStatus:(status:BookmarkStatus)=>void;state:AppState;update:(fn:(s:AppState)=>AppState)=>void;t:(ko:string,en:string)=>string;storageError:boolean;notice:string;notify:(message:string)=>void};
...
export function Provider({children,storageKey='ncg:v1'}:{children:ReactNode;storageKey?:string}){
  ...
  useSyncExternalStore(subscribeInterface,interfaceSnapshot,interfaceSnapshot);
  const interfaceStatus:InterfaceStatus=hasInterfaceTranslation(state.language)?'bundled':runtimeInterfaceStatus(state.ui);
  ...
  return <Context.Provider value={{interfaceStatus,retryInterface:()=>retryInterface(state.ui),state,update,t,storageError,notice,notify:setNotice,bookmarkStatus,setBookmarkStatus,quizStatus,setQuizStatus,videoBookmarkStatus,setVideoBookmarkStatus}}>{children}</Context.Provider>;
}
