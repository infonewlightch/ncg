...
type Auth={session:Session|null;loading:boolean;error:boolean;configured:boolean;issue:AuthIssue|null;clearIssue:()=>void};
const Context=createContext<Auth>({session:null,loading:true,error:false,configured:false,issue:null,clearIssue:()=>{}});
export function AuthProvider({children}:{children:ReactNode}){
 ...
 useEffect(()=>{
  ...
  const {data:{subscription}}=supabase.auth.onAuthStateChange((event,value)=>{if(alive){eventRevision++;setSession(value);if(event!=='INITIAL_SESSION')setLoading(false);}});
  (async()=>{
 ...
 return <Context.Provider value={{session,loading,error:Boolean(issue),configured:Boolean(supabase),issue,clearIssue:()=>setIssue(null)}}>{children}</Context.Provider>;
}
